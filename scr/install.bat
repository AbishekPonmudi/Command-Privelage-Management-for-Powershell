...
set /p username=New username (must be atleast 3 characters long): 
...
for /f "delims=" %%P in ('powershell -command "$p = read-host 'New password (Password not must not contain username):' -AsSecureString; [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($p))"') do set "password=%%P"
