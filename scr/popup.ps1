...
function Prompt-ForCredentials {
    ...
    try {
        ...
        if ($credential -eq $null) {
            Write-warning "Credential prompt canceled. PowerShell remains usable. "
        ...
        Write-warning "[windows require password]"
    }
}
...
try {
    ...
    while (-not $authenticated) {
        ...
        if (-not $authenticated) {
            $attempts++
            if ($attempts -ge 3) {
                Write-warning "Maximum attempts reached. Opening popup window for credential entry."
                $authenticated = Prompt-ForCredentials -attempt $attempts
            }
            if ($attempts -ge 3) {
